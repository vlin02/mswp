...
export function satisfy(conds, { maxDepth = 2 } = {}) {
    let iter = 0;
    ...
    const allResolved = {};
    conds.forEach((cond) => cond.seq.forEach((el) => (atElement[el] = new Set())));
    function addToQueue(cond, resolved) {
        const pos = cond.cnt === cond.seq.length ? true : cond.cnt === 0 ? false : null;
        if (pos != null) {
            cond.seq.forEach((el) => (resolved[el] = pos));
            return false;
        }
        const key = ops.shash(cond.seq);
        if (visitedHashes.has(key))
            return false;
        visitedHashes.add(key);
        const cond1 = { ...cond, key, toDel: [] };
        queue.push(cond1);
        cond.seq.forEach((el) => atElement[el].add(cond1));
        return true;
    }
    function pruneResolved(resolved) {
        const reducableConds = new Set();
        ...
        reducableConds.forEach((nbr) => {
            const s1 = ops.difference(nbr.seq, nbr.toDel);
            nbr.seq = s1;
    ...
    while (queue.length > 0) {
        iter += 1;
        const cond = queue.pop();
        if (obsoleteConds.has(cond))
            continue;
        const nbrs = new Set();
        ...
        nbrs.forEach((nbr) => {
            ...
            if (cond === nbr)
                return;
            const [lo, hi] = cond.seq.length < nbr.seq.length ? [cond, nbr] : [nbr, cond];
